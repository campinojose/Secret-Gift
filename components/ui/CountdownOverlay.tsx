...
export default function CountdownOverlay() {
  const [remaining, setRemaining] = useState(() => getRemaining(nextTargetDate(), new Date()));
  const [visible, setVisible] = useState(remaining.totalMs > 0);

  useEffect(() => {
    const interval = setInterval(() => {
      const now = new Date();
      const rem = getRemaining(nextTargetDate(), now);
      setRemaining(rem);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black text-white flex flex-col items-center justify-center p-6">
      <div className="max-w-3xl w-full text-center">
        ...
        <p className="mb-6 text-slate-300">Volveremos a las {formatTargetTime()} — mientras tanto, paciencia 🤍</p>

        <div className="grid grid-cols-5 gap-4 text-center">
          <TimeBox value={remaining.months} label="Meses" />
          <TimeBox value={remaining.days} label="Días" />
          <TimeBox value={remaining.hours} label="Horas" />
          <TimeBox value={remaining.minutes} label="Minutos" />
          <TimeBox value={remaining.seconds} label="Segundos" />
        </div>
...
function formatTargetTime() {
  const t = nextTargetDate();
  return t.toLocaleString(undefined, { day: "2-digit", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" });
}
